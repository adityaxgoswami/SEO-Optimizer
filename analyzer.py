...
#         "pass_message": "The page has a meta description within the optimal length (50-160 characters).",
#         "fail_messages": {
#             "meta_description_missing": "The page is missing a meta description, which can lead to lower click-through rates.",
#             "meta_description_too_short": "The meta description is too short. It should be at least 50 characters to provide sufficient detail.",
#             "meta_description_too_long": "The meta description is too long. Descriptions over 160 characters may be truncated in search results."
#         },
#         "recommendation": "Write a unique meta description between 50 and 160 characters that summarizes the page and encourages clicks."
#     },
#     "h1_heading": {
...
#     },
#     "nofollow_tag": {
#         "feature_name": "Nofollow Tag Test", "category": "Technical",
#         "description": "Checks if the meta robots tag contains a 'nofollow' directive, which stops search engines from following any links on the page.",
#         "pros": "Can be used strategically to conserve crawl budget, but is generally not recommended for pages you want to rank.",
#         "pass_message": "This page does not use the 'nofollow' meta tag.",
#         "fail_messages": {
#             "meta_robots_nofollow": "This page has a \"nofollow\" meta tag, which prevents search engines from following links. This can harm your internal linking strategy."
#         },
#         "recommendation": "Remove the 'nofollow' directive from the meta robots tag unless you have a specific strategic reason to prevent link equity from flowing from this page."
#     },
#     "mixed_content": {
...
#     },
#     "charset_declaration": {
#         "feature_name": "Charset Declaration Test", "category": "Technical",
#         "description": "Checks for the presence of a character encoding declaration (like UTF-8), which ensures text displays correctly.",
#         "pros": "A charset declaration prevents text from being garbled and ensures browsers render special characters properly.",
#         "pass_message": "The page has a character encoding declaration.",
#         "fail_messages": {
#             "charset_missing": "The character encoding (e.g., UTF-8) is not declared. This can lead to text rendering incorrectly."
#         },
#         "recommendation": "Add a meta charset tag to the <head> section of your HTML, e.g., <meta charset=\"UTF-8\">."
#     },
#     "ads_txt": {
#         "feature_name": "Ads.txt Validation Test", "category": "Technical",
#         "description": "Checks for a valid ads.txt file, which lists authorized digital sellers of your advertising inventory.",
#         "pros": "An ads.txt file helps prevent ad fraud and ensures you receive revenue from legitimate advertisers.",
#         "pass_message": "An ads.txt file was found at the root of the domain.",
#         "fail_messages": {
#             "ads_txt_missing": "An ads.txt file was not found. If you run programmatic ads, this is essential to prevent ad fraud."
#         },
#         "recommendation": "If you display ads on your site, create an ads.txt file listing your authorized ad sellers and place it in your root directory."
#     },
#     "url_redirects": {
#         "feature_name": "URL Redirects Test", "category": "Technical",
#         "description": "Checks if the requested URL redirects to a different URL.",
#         "pros": "Redirects are necessary for moving content, but a redirect chain (multiple redirects) can slow down load times and confuse crawlers.",
#         "pass_message": "The URL does not have any redirects and loads directly.",
#         "fail_messages": {
#             "url_has_redirect": "The requested URL redirects. This is acceptable, but direct links are always faster.",
#             "url_has_redirect_chain": "The URL goes through a redirect chain (multiple redirects), which can harm performance and SEO."
#         },
#         "recommendation": "If a redirect is present, ensure it's a single 301 (permanent) redirect. Avoid redirect chains by updating all internal links to point directly to the final destination URL."
#     },
#     "spf_records": {
#         "feature_name": "SPF Records Test", "category": "Technical",
#         "description": "Checks for a Sender Policy Framework (SPF) record in your DNS, which helps prevent email spoofing.",
#         "pros": "An SPF record specifies which mail servers are allowed to send email from your domain, improving email deliverability and security.",
#         "pass_message": "A valid SPF record was found for the domain.",
#         "fail_messages": {
#             "spf_record_missing": "No SPF record was found for this domain. This can lead to email deliverability issues and makes your domain vulnerable to email spoofing.",
#             "spf_check_error": "An error occurred while checking for the SPF record."
#         },
#         "recommendation": "Add a TXT record to your domain's DNS settings with a valid SPF policy (e.g., 'v=spf1 a mx ~all')."
#     },
#     "media_query_test": {
...
#     "meta_robots_noindex": "Critical", "meta_robots_nofollow": "Medium","images_missing_alt_critical": "Critical",
...
#     "h1_missing": "High", "canonical_missing": "High",
...
#     "url_has_redirect_chain": "Medium",
#     # Low
#     "spf_record_missing": "Medium",
#     "spf_check_error": "Low",
...
#     meta_desc_text = seo_data.get("meta_description")
#     if not meta_desc_text or meta_desc_text == "No Meta Description Found":
#         findings["meta_description_missing"] = {}
#     else:
#         desc_len = len(meta_desc_text)
#         # Using the 50-160 char range from your existing recommendation
#         if desc_len < 50:
#             findings["meta_description_too_short"] = {"value": f"{desc_len} characters"}
#         elif desc_len > 160:
#             findings["meta_description_too_long"] = {"value": f"{desc_len} characters"}
...
#     site_files = seo_data.get("site_files", {})
...
#     if not site_files.get("has_ads_txt"):
#         findings["ads_txt_missing"] = {}
    
#     #URL redirects check  
#     redirect_chain = seo_data.get("redirect_chain", [])
#     if len(redirect_chain) > 1:
#         findings["url_has_redirect_chain"] = {"value": f"{len(redirect_chain)} redirects", "details": [f"{r['status_code']} -> {r['url']}" for r in redirect_chain]}
#     elif len(redirect_chain) == 1:
#         findings["url_has_redirect"] = {"value": "1 redirect", "details": [f"{redirect_chain[0]['status_code']} -> {redirect_chain[0]['url']}"]}
#         spf_check = seo_data.get("spf_record_check", {})
#     if spf_check.get("status") == "missing":
#         findings["spf_record_missing"] = {"value": spf_check.get("error", "No record found.")}
#     elif spf_check.get("status") == "error":
#         findings["spf_check_error"] = {"value": spf_check.get("error", "Check failed.")}
#     if not seo_data.get("canonical"): findings["canonical_missing"] = {}
#     meta_robots_content = seo_data.get("meta_robots", "").lower()
    
#     # Check for noindex/nofollow directives --> new feature
#     if "noindex" in meta_robots_content:
#         findings["meta_robots_noindex"] = {}
#     if "nofollow" in meta_robots_content:
#         findings["meta_robots_nofollow"] = {}

#     if seo_data.get("deprecated_tags"): findings["deprecated_tags"] = {"value": ", ".join(seo_data.get("deprecated_tags"))}
    
#     # New charset check
#     if not seo_data.get("charset"):
...
FEATURE_ANALYSIS_MAP = {
    ...
    "spell_check": {
        "feature_name": "Grammar and Spelling (AI Check)", "category": "Content",
        "description": "Performs an advanced grammar and spell check (Grammarly-like) on the page's body text.",
        "pros": "Correct grammar, spelling, and professional style enhance user trust and authority.",
        "pass_message": "No significant spelling or grammar errors were detected.",
        "fail_messages": {
            "spell_check_misspelled": "Misspelled words were detected on the page.",
            "grammar_issues_found": "Grammar or style issues were detected.",
            "spell_check_error": "The spell/grammar check could not run (library missing or error)."
        },
        "recommendation": "Review the content for the highlighted spelling and grammar errors. Consistent, error-free text builds trust."
    },
    "https_usage": {
        "feature_name": "HTTPS Usage", "category": "Technical",
        "description": "HTTPS encrypts data between a user's browser and your website, ensuring security and trust.",
...
IMPACT_LEVELS = {
    ...
    "meta_refresh_found": "Medium", "custom_404_missing": "Medium",
    "keyword_missing_opening_paragraph": "Medium", "http2_missing": "Medium", "unsafe_links_found": "Medium",
    "url_has_redirect_chain": "Medium", "grammar_issues_found": "Medium",
    # Low
    "spf_record_missing": "Medium",
    "spf_check_error": "Low",
    "header_structure_issue": "Low", "readability_low": "Low", "text_ratio_low": "Low",
...
def generate_seo_report(seo_data: dict, target_keyword: str = "") -> dict:
    ...
    # Updated Grammar/Spell Check Findings
    spell_data = seo_data.get("spell_check", {})
    if spell_data.get("error"):
        findings["spell_check_error"] = {"value": spell_data["error"]}
    else:
        if spell_data.get("misspelled_count", 0) > 0:
            details = [f"{w['word']} (Try: {', '.join(w['suggestions'])})" for w in spell_data.get("misspelled_words", [])[:3]]
            findings["spell_check_misspelled"] = {
                "value": f"{spell_data['misspelled_count']} issues",
                "details": details
            }
        
        # New Grammar Check findings
        grammar_count = spell_data.get("grammar_issues_count", 0) + spell_data.get("style_issues_count", 0)
        if grammar_count > 0:
            # Combine issues for details
            all_grammar = spell_data.get("grammar_issues", []) + spell_data.get("style_issues", [])
            details = [f"{i['message']} (Context: '...{i['context'][i['offset']-5 : i['offset']+i['length']+5]}...')" for i in all_grammar[:3]]
            findings["grammar_issues_found"] = {
                "value": f"{grammar_count} potential grammar/style issues",
                "details": details
            }
